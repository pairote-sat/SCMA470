...
2. The compound negative binomial distribution is an appropriate to model the heterogeneity of the numbers of claims occurring for
...
numclaims <- sapply(lambda,rpois,n =1)
hist(numclaims)
print(mean(numclaims))
print(var(numclaims))

#Mixture ~ nb(alpha,beta/(beta+1))
alphaMixture <- alpha
betaMixture <- beta/(beta+1)
meanMixture <- alphaMixture*(1-betaMixture)/betaMixture
varMixture <- meanMixture/betaMixture
```
...
ES <- lambda*m1
VS <- lambda*m2
SKS <- (lambda * m3)/(lambda * m2)^(3/2)

theta <- qnorm(0.95)*sqrt(VS)/ES
P <- (1+theta)*ES
...
betaTG <- sqrt(alphaTG/VS)

kTG <- ES - alphaTG/betaTG 

qGamma <- qgamma(0.95,shape = alphaTG, rate = betaTG)

thetaTG <- (qGamma + kTG)/ES - 1
PTG <- (1+thetaTG)*ES

theta2 <- qnorm(0.95)
P2 <- ES + theta2*sqrt(VS)
```


The loss distribution has Pareto distribution $X \sim Pa(4,3)$. Therefore, $\mathrm{E}[X] = `r m1`$ and $\mathrm{E}[X^2] = `r m2`$.

The mean and variance of the aggregate claim amounts are $\mathrm{E}[S] = \lambda \mathrm{E}[X] = `r ES`$
and $\mathrm{Var}[S] = \lambda \mathrm{E}[X^2] = `r VS`$.

1. Use the normal approximation $S \sim \mathcal{N}(`r ES`, `r VS`)$.
...
$$P = E[S] + \theta\, E[S] = ( 1 + \theta)E[S] = ( 1 + \theta)(`r ES`).$$
\begin{align}
0.95 &= \Pr( P - S > 0)  \\
&= \Pr( ( 1 + \theta)(`r ES`) - S > 0)  \\
&= \Pr( S < ( 1 + \theta)(`r ES`) ) \\
&= \Pr( Z < \frac{( 1 + \theta)(`r ES`) - `r ES` }{\sqrt{`r VS`}} ) \\
&= \Pr( Z < \frac{( \theta)(`r ES`) }{\sqrt{`r VS`}} ).
\end{align}
This gives $\frac{( \theta)(`r ES`) }{\sqrt{`r VS`}} = `r qnorm(0.95)`$ and 
$$\theta = `r theta`, \quad  P = `r round(P,3)`.$$ 
Instead of using the normal approximation to aggregate claims $S$, we now assume that $S$ is approximated by $Y + k$ where
$Y \sim \mathcal{G}(`r alphaTG`, `r betaTG`)$ and $k = `r kTG`$ is a constant. It follows that
\begin{align}
0.95 &= \Pr( P - S > 0)  \\
&= \Pr( ( 1 + \theta)(`r ES`) - S > 0)  \\
&= \Pr( S < ( 1 + \theta)(`r ES`) ) \\
&= \Pr( Y < ( 1 + \theta)(`r ES`) - `r kTG` ) \\
\end{align}

Therefore, $(1 + \theta)$(`r ES`) - `r kTG` =  qgamma(0.95,shape = `r round(alphaTG,6)`, rate = `r round(betaTG,6)`) = `r round(qGamma,6)`, which results in
$$\theta = `r thetaTG`, \quad  P = `r round(PTG,3)`.$$ 


2. Using the standard deviation principle (SVP), we set 
$$P = E[S] + \theta\, \text{SD}[S] = `r ES` + \theta \cdot \sqrt{`r VS`}$$
For the normal approximation of the aggregate claims, we obtain
\begin{align}
0.95 &= \Pr( P - S > 0)  \\
&= \Pr( `r ES` + \theta \cdot \sqrt{`r VS`} - S > 0)  \\
&= \Pr( S < `r ES` + \theta \cdot \sqrt{`r VS`} ) \\
&= \Pr( Z < \frac{`r ES` + \theta \cdot \sqrt{`r VS`} - `r ES` }{\sqrt{`r VS`}} ) \\
&= \Pr( Z < \theta).
\end{align}

Hence, $\theta = `r round(theta2,6)`$ and $P = `r round(P2,3)`$.

**Comment** The loading factors (for normal approximation) in both two cases are different because they are applied to different quantities (i.e. $\mathrm{E}[S]$ and $\mathrm{Var}[S]$). But they give the same premium.
